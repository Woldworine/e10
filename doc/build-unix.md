...
memory available when compiling Coon Core. With 512MB of memory or less
...
If you want to build Coon-Qt, make sure that the required packages for Qt development
...
Once these are installed, they will be found by configure and a Coon-qt executable will be
built by default.
...
make install
```
### Compiling  Coon-Qt wallet

Clone the coon repository
```
git clone https://github.com/coon/coonwallet.git
```
if required, fix the leveldb files permissions
```
cd ~/coonwallet/src/leveldb
...
go back to coon dir to modify coon-qt.pro if needed :
```
cd ~/coonwallet
nano coon-qt.pro
```
...
### Compiling coond
With dependencies (except QT cause not needed here) compliled as above, put these lines in ~/coonwallet/src/makefile.unix (first lines)
```
DEPS_PATH = $(HOME)/deps
SECP256K1_LIB_PATH = src/secp256k1/.libs
SECP256K1_INCLUDE_PATH = src/secp256k1/include
