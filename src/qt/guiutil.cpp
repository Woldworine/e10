...
{
    // Convert coon:// to coon:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("coon://"))
    {
        uri.replace(0, 12, "coon:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "coon.lnk";
}
...
{
    return GetAutostartDir() / "coon.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("coon-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  coon-qt [" + tr("command-line options") + "]                     " + "\n";
